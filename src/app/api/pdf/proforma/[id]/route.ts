...
function buildPdf(proforma: Record<string, unknown>): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    const phone = t?.phone_number
      ...
      : null;
    const taxAmt   = ((proforma.subtotal as number) - (proforma.discount as number)) * ((proforma.tax_rate as number) / 100);
    const dotColor = STATUS_COLOR[proforma.status as string] ?? TEAL;
    ...
    doc.font("Helvetica-Bold").fontSize(28).fillColor(DARK)
      .text("PROFORMA", 0, y, { width: rightX, align: "right" });
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const dotTextWidth = (doc as any).widthOfString((proforma.status as string).toUpperCase(), { fontSize: 9 });
    ...
    doc.font("Helvetica-Bold").fontSize(9).fillColor(dotColor)
      .text((proforma.status as string).toUpperCase(), dotX + 10, statusY, { lineBreak: false });
    ...
    // ── PROFORMA FOR / CONSULTANT ────────────────────────────────────────────
    const colW = CONTENT_W * 0.46;

    doc.font("Helvetica-Bold").fontSize(8).fillColor(TEAL).text("PROFORMA FOR", ML, y);
    y += 14;
    ...
    if ((proforma.tax_rate as number) > 0) {
      doc.font("Helvetica").fontSize(9).fillColor(GRAY)
    ...
    if (proforma.terms) {
      ...
      doc.font("Helvetica").fontSize(8.5).fillColor(GRAY)
        .text(String(proforma.terms), ML, y + 14, { width: CONTENT_W / 2 - 8, lineGap: 4 });
    }
...
export async function GET(
  ...
  try {
    ...
    console.log("[pdf/proforma] fetching proforma", id);
    ...
    if (error || !data) {
      console.error("[pdf/proforma] supabase error:", error);
      return NextResponse.json({ error: "Not found" }, { status: 404 });
    }

    console.log("[pdf/proforma] building PDF for", (data as Record<string, unknown>).number);
    const buffer = await buildPdf(data as Record<string, unknown>);
    console.log("[pdf/proforma] PDF rendered, size:", buffer.length, "bytes");
    ...
    console.error("[pdf/proforma] FATAL ERROR:");
