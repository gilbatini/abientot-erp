...
import { createInvoice, updateInvoice, type ItemDraft, type InvoiceWithItems } from "@/actions/invoices";
...
export function InvoiceForm({ travellers, initialData }: Props) {
  ...
  const [items,       setItems]       = useState<ItemDraft[]>(
    initialData?.invoice_items?.length
      ? initialData.invoice_items.map(it => ({
  ...
  function updateItem<K extends keyof ItemDraft>(idx: number, key: K, val: ItemDraft[K]) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (initialData) {
        await updateInvoice(initialData.id, payload, filledItems);
      } else {
        await createInvoice(payload, filledItems);
      }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex gap-3 justify-end">
        ...
        <Button type="submit" disabled={saving}>{saving ? "Saving…" : initialData ? "Update Invoice" : "Create Invoice"}</Button>
      </div>
    </form>
  );
}
