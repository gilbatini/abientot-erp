...
import { createProforma, updateProforma, type ProformaItemDraft, type ProformaWithItems } from "@/actions/proformas";
...
export function ProformaForm({ travellers, initialData }: Props) {
  ...
  const [items,       setItems]       = useState<ProformaItemDraft[]>(
    initialData?.proforma_items?.length
      ? initialData.proforma_items.map(it => ({
  ...
  function updateItem<K extends keyof ProformaItemDraft>(idx: number, key: K, val: ProformaItemDraft[K]) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (initialData) {
        await updateProforma(initialData.id, payload, filledItems);
      } else {
        await createProforma(payload, filledItems);
      }
      router.push("/proformas");
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to save proforma");
      setSaving(false);
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex gap-3 justify-end">
        <Button type="button" variant="secondary" onClick={() => router.push("/proformas")}>Cancel</Button>
        <Button type="submit" disabled={saving}>{saving ? "Saving…" : initialData ? "Update Proforma" : "Create Proforma"}</Button>
      </div>
    </form>
  );
}
